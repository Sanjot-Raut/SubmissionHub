...
def store_image_features(assignment_id, image_paths, new_features):
    ...
    try:
        ...
        for page_number, (img_path, features) in enumerate(zip(image_paths, new_features)):
            ...
            VALUES (%s, %s, %s)
            """
            cursor.execute(insert_query, (assignment_id, page_number, features_blob.getvalue()))
...
def retrieve_stored_features(student_assignment_id):
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            stored_features.append((assignment_id, features))
...
def compare_features(new_features, stored_features):
    ...
    for new_feature in new_features:
        for stored_feature in stored_features:
            similarity = cosine_similarity([new_feature], [stored_feature[1]])
            similarities.append((stored_feature[0], similarity[0][0]))
...
def filter_ids_with_all_high_scores(data, threshold=0.90):
...
def check_assignment_similarity(new_features, student_assignment_id):
    stored_features = retrieve_stored_features(student_assignment_id)
    similarities = compare_features(new_features, stored_features)
...
def assignment_submission_process(assignment_id,student_id,file_path,comments):
